// Trânsito da Nova Cidade Ludigins — carros reais, malha asfaltada e orientação segura.
(function(){
...
/* Coordenadas recalibradas pela arte oficial 1536x864. Os nós abaixo ficam NO CENTRO DO ASFALTO. A malha usa somente cruzamentos e ruas; nunca cruza lotes, prédios, jardins ou calçadas. */
const P={
T0:[30.4,3.3],T1:[49.5,3.3],T2:[67.9,3.3],T3:[84.8,3.3],
A0:[30.4,24.9],A1:[49.5,24.9],A2:[67.9,24.9],A3:[84.8,24.9],
B0:[30.4,47.8],B1:[49.5,47.8],B2:[67.9,47.8],B3:[84.8,47.8],
C0:[30.4,67.5],C1:[49.5,67.5],C2:[67.9,67.5],C3:[84.8,67.5],
D0:[30.4,88.3],D1:[49.5,88.3],D2:[67.9,88.3],D3:[84.8,88.3]
};
const edges=[];['T','A','B','C','D'].forEach(r=>{for(let i=0;i<3;i++)edges.push([r+i,r+(i+1)])});for(let i=0;i<4;i++){edges.push(['T'+i,'A'+i],['A'+i,'B'+i],['B'+i,'C'+i],['C'+i,'D'+i])}
...
function aplicarModelo(perfil={}){modeloAtual=escolherModelo(perfil);taxiImg.src=modeloAtual.arquivo+'?v=20260924-3';taxiImg.alt='Táxi '+modeloAtual.nome;taxi.dataset.modelo=String(modeloAtual.nivel);taxi.title='Táxi '+modeloAtual.nome;return modeloAtual}
...
/* O desenho do carro nunca fica de cabeça para baixo. Em vias horizontais apenas espelha; nas verticais mantém a lateral visível com inclinação mínima. */
function orientar(de,para){const dx=para[0]-de[0],dy=para[1]-de[1];let sx=1,ang=0;if(Math.abs(dx)>=Math.abs(dy)){sx=dx>=0?1:-1}else{sx=dy>=0?1:-1;ang=dy>=0?7:-7}taxi.style.transform=`translate(-50%,-50%) scaleX(${sx}) rotate(${ang}deg)`}
/* Velocidade baseada na distância: aproximadamente 4,2 s para atravessar um quarteirão normal. Assim trechos longos não viram saltos rápidos. */
function animarTrecho(de,para,cb){orientar(de,para);const dx=para[0]-de[0],dy=para[1]-de[1],dist=Math.hypot(dx,dy);const dur=Math.max(2200,dist*190);taxi.style.transition=`left ${dur}ms linear, top ${dur}ms linear`;requestAnimationFrame(()=>{posXY(para[0],para[1]);setTimeout(cb,dur+40)})}
