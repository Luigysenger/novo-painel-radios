...
.cidade-transito{position:absolute;inset:0;z-index:7;pointer-events:none;overflow:hidden}.cidade-veiculo{position:absolute;left:0;top:0;transform:translate(-50%,-50%);width:clamp(105px,7.4vw,142px);height:clamp(62px,4.5vw,86px);z-index:8;transform-origin:center center;will-change:left,top,transform}.cidade-veiculo:before{content:'';position:absolute;left:50%;top:50%;width:82%;aspect-ratio:1;border:4px solid #ffd400;border-radius:50%;transform:translate(-50%,-50%);background:rgba(255,212,0,.13);box-shadow:0 0 8px #ffd400,0 0 18px rgba(255,212,0,.9),inset 0 0 12px rgba(255,212,0,.4);z-index:-1}.cidade-veiculo img{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;object-position:center;filter:drop-shadow(0 4px 6px #000);user-select:none;-webkit-user-drag:none}.cidade-passageiro{position:absolute;transform:translate(-50%,-105%);font-size:clamp(14px,1.45vw,23px);z-index:9;pointer-events:auto;cursor:pointer;filter:drop-shadow(0 2px 2px #000)}.cidade-passageiro b{display:block;background:#101827e8;color:#fff;border:2px solid #facc15;border-radius:999px;padding:2px 6px;font:900 clamp(7px,.65vw,10px) Arial;white-space:nowrap}.cidade-status{position:absolute;left:50%;top:2%;transform:translateX(-50%);background:#07101def;border:1px solid #475569;border-radius:999px;padding:6px 10px;color:#e2e8f0;font:900 clamp(8px,.8vw,11px) Arial;z-index:10;white-space:nowrap}.cidade-status.corrida{background:#3b0764ef;border-color:#c084fc;color:#fff}@media(max-width:650px){.cidade-veiculo{width:clamp(78px,18vw,105px);height:clamp(48px,11vw,66px)}}
...
/* MALHA VIÁRIA medida sobre a arte NOVA da Cidade Ludigins. Os nós ficam no centro do ASFALTO, nos cruzamentos. Não há segmentos diagonais nem atalhos por terrenos. */
const P={
A0:[26.9,18.0],A1:[43.0,18.0],A2:[58.1,18.0],A3:[73.0,18.0],A4:[88.0,18.0],
B0:[26.9,42.0],B1:[43.0,42.0],B2:[58.1,42.0],B3:[73.0,42.0],B4:[88.0,42.0],
C0:[26.9,61.0],C1:[43.0,61.0],C2:[58.1,61.0],C3:[73.0,61.0],C4:[88.0,61.0],
D0:[26.9,82.0],D1:[43.0,82.0],D2:[58.1,82.0],D3:[73.0,82.0],D4:[88.0,82.0]
};
const rows=[['A0','A1','A2','A3','A4'],['B0','B1','B2','B3','B4'],['C0','C1','C2','C3','C4'],['D0','D1','D2','D3','D4']];
const edges=[];rows.forEach(row=>{for(let i=0;i<row.length-1;i++)edges.push([row[i],row[i+1]])});for(let c=0;c<5;c++)for(let r=0;r<3;r++)edges.push([rows[r][c],rows[r+1][c]]);
const adj={};Object.keys(P).forEach(k=>adj[k]=[]);edges.forEach(([a,b])=>{adj[a].push(b);adj[b].push(a)});
const taxi=document.getElementById('taxiCidade'),taxiImg=document.getElementById('taxiCidadeImagem'),status=document.getElementById('cidadeTransitoStatus');let atual='A3',movendo=false,corridaAtiva=false,modeloAtual=CARROS[0];
...
function aplicarModelo(perfil={}){modeloAtual=escolherModelo(perfil);taxiImg.src=modeloAtual.arquivo+'?v=20260924-6';taxiImg.alt='Táxi '+modeloAtual.nome;taxi.dataset.modelo=String(modeloAtual.nivel);taxi.title='Táxi '+modeloAtual.nome;return modeloAtual}
...
/* O carro sempre aponta no sentido da rua. Horizontal: lateral direita/esquerda. Vertical: gira apenas 90/-90 graus; nunca fica invertido. */
...
/* Velocidade urbana lenta. O carro termina um trecho antes de iniciar a curva seguinte. */
function animarTrecho(de,para,cb){orientar(de,para);const dist=Math.hypot(para[0]-de[0],para[1]-de[1]);const dur=Math.max(7800,dist*520);taxi.style.transition=`left ${dur}ms linear,top ${dur}ms linear,transform 320ms ease`;requestAnimationFrame(()=>{posXY(para[0],para[1]);setTimeout(cb,dur+80)})}
...
const corridas=[{o:'A1',d:'C3',v:4,n:'Hospital → Academia'},{o:'A2',d:'D2',v:3,n:'Bombeiros → Bairro Central'},{o:'B0',d:'B4',v:5,n:'Bairro das Flores → Lanchonete'},{o:'B2',d:'C0',v:2,n:'Prefeitura → Bairro das Flores'},{o:'B4',d:'D1',v:6,n:'Lanchonete → Bairro Lago Sul'},{o:'C0',d:'A3',v:5,n:'Bairro das Flores → Posto'},{o:'C2',d:'A1',v:4,n:'Loja de Roupas → Hospital'},{o:'C4',d:'B1',v:6,n:'Indústria → Supermercado'},{o:'D1',d:'B2',v:3,n:'Bairro Lago Sul → Prefeitura'},{o:'D2',d:'B3',v:4,n:'Bairro Central → Restaurante'},{o:'D4',d:'A2',v:5,n:'Nova Esperança → Bombeiros'}];
