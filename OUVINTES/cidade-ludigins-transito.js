...
/* MALHA VIÁRIA: coordenadas medidas diretamente sobre o mapa oficial. Todos os pontos são centros do asfalto. O táxi só pode viajar pelos segmentos abaixo. */
const P={
R0C0:[24.9,18.2],R0C1:[40.0,18.2],R0C2:[57.0,18.2],R0C3:[74.1,18.2],R0C4:[87.4,18.2],
R1C0:[24.9,40.8],R1C1:[40.0,40.8],R1C2:[57.0,40.8],R1C3:[74.1,40.8],R1C4:[87.4,40.8],
R2C0:[24.9,59.0],R2C1:[40.0,59.0],R2C2:[57.0,59.0],R2C3:[74.1,59.0],R2C4:[87.4,59.0],
R3C0:[31.0,79.2],R3C1:[40.0,79.2],R3C2:[57.0,79.2],R3C3:[74.1,79.2],R3C4:[87.4,79.2]
};
const edges=[];for(let r=0;r<4;r++){const ini=r===3?0:0;for(let c=0;c<4;c++){const a=`R${r}C${c}`,b=`R${r}C${c+1}`;if(P[a]&&P[b])edges.push([a,b])}}for(let c=0;c<5;c++){for(let r=0;r<3;r++){const a=`R${r}C${c}`,b=`R${r+1}C${c}`;if(P[a]&&P[b])edges.push([a,b])}}
...
function aplicarModelo(perfil={}){modeloAtual=escolherModelo(perfil);taxiImg.src=modeloAtual.arquivo+'?v=20260924-4';taxiImg.alt='Táxi '+modeloAtual.nome;taxi.dataset.modelo=String(modeloAtual.nivel);taxi.title='Táxi '+modeloAtual.nome;return modeloAtual}
...
/* Direção real: horizontal mostra a lateral; vertical gira 90 graus para apontar para cima/baixo. Nunca usa 180 graus, portanto nunca fica de cabeça para baixo. */
...
/* Movimento lento e contínuo. Cada quarteirão leva vários segundos; sem teleporte e sem diagonal. */
function animarTrecho(de,para,cb){orientar(de,para);const dx=para[0]-de[0],dy=para[1]-de[1],dist=Math.hypot(dx,dy);const dur=Math.max(4300,dist*300);taxi.style.transition=`left ${dur}ms linear,top ${dur}ms linear,transform 260ms ease`;requestAnimationFrame(()=>{posXY(para[0],para[1]);setTimeout(cb,dur+50)})}
