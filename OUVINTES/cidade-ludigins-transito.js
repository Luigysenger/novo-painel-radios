...
/* MALHA REAL DA ARTE: coordenadas medidas novamente no centro das faixas de ASFALTO visíveis. Só existem trechos ortogonais entre cruzamentos; não há atalhos por calçadas, terrenos, praças ou prédios. */
const P={
A0:[30.8,18.4],A1:[49.1,18.4],A2:[68.0,18.4],A3:[87.2,18.4],
B0:[30.8,43.7],B1:[49.1,43.7],B2:[68.0,43.7],B3:[87.2,43.7],
C0:[30.8,63.7],C1:[49.1,63.7],C2:[68.0,63.7],C3:[87.2,63.7],
D0:[30.8,86.8],D1:[49.1,86.8],D2:[68.0,86.8],D3:[87.2,86.8]
};
const rows=[['A0','A1','A2','A3'],['B0','B1','B2','B3'],['C0','C1','C2','C3'],['D0','D1','D2','D3']];
const edges=[];rows.forEach(row=>{for(let i=0;i<row.length-1;i++)edges.push([row[i],row[i+1]])});for(let c=0;c<4;c++)for(let r=0;r<3;r++)edges.push([rows[r][c],rows[r+1][c]]);
const adj={};Object.keys(P).forEach(k=>adj[k]=[]);edges.forEach(([a,b])=>{adj[a].push(b);adj[b].push(a)});
const taxi=document.getElementById('taxiCidade'),taxiImg=document.getElementById('taxiCidadeImagem'),status=document.getElementById('cidadeTransitoStatus');let atual='A2',movendo=false,corridaAtiva=false,modeloAtual=CARROS[0];
...
function aplicarModelo(perfil={}){modeloAtual=escolherModelo(perfil);taxiImg.src=modeloAtual.arquivo+'?v=20260924-7';taxiImg.alt='Táxi '+modeloAtual.nome;taxi.dataset.modelo=String(modeloAtual.nivel);taxi.title='Táxi '+modeloAtual.nome;return modeloAtual}
...
/* Velocidade aumentada em aproximadamente 3 níveis sobre a regulagem anterior (escala visual 0–10), mantendo condução suave. */
function animarTrecho(de,para,cb){orientar(de,para);const dist=Math.hypot(para[0]-de[0],para[1]-de[1]);const dur=Math.max(5200,dist*350);taxi.style.transition=`left ${dur}ms linear,top ${dur}ms linear,transform 260ms ease`;requestAnimationFrame(()=>{posXY(para[0],para[1]);setTimeout(cb,dur+60)})}
...
const corridas=[{o:'A0',d:'C2',v:4,n:'Hospital → Academia'},{o:'A1',d:'D1',v:3,n:'Bombeiros → Bairro Central'},{o:'B0',d:'B3',v:5,n:'Bairro das Flores → Lanchonete'},{o:'B1',d:'C0',v:2,n:'Prefeitura → Bairro das Flores'},{o:'B3',d:'D0',v:6,n:'Lanchonete → Bairro Lago Sul'},{o:'C0',d:'A2',v:5,n:'Bairro das Flores → Posto'},{o:'C1',d:'A0',v:4,n:'Loja de Roupas → Hospital'},{o:'C3',d:'B0',v:6,n:'Indústria → Supermercado'},{o:'D0',d:'B1',v:3,n:'Bairro Lago Sul → Prefeitura'},{o:'D1',d:'B2',v:4,n:'Bairro Central → Restaurante'},{o:'D3',d:'A1',v:5,n:'Nova Esperança → Bombeiros'}];
